...
- **Review mode (audio only):** Tab bar shows Preview (accent, Play/Pause with 3D flip) + Delete (red trash icon). Cloud layer: review timer at `bottomLeading` (`00:00:00 — 00:05:23`), Save Dream button at `bottomTrailing`. Waveform shows playback animation when Preview is pressed — scrolls through recorded bars from beginning to end at 60fps, synced with audio duration. Editable `TextEditor` with transcript from speech recognition (placeholder "Add dream description..." when empty). Saved as audio dream with transcript.
