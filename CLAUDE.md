...
- **Glass effect:** Always use `.reveriGlass(.circle)` or `.reveriGlass(.capsule)` for glass styling. Never use raw `.glassEffect()` or custom `.background` + `.clipShape` combos. Multiple adjacent glass elements → `GlassEffectContainer(spacing:)`. Overlapping elements (negative spacing) → NO container, use independent glass + `zIndex`.
