...
- **Journal header:** Black background + blur gradient orb (orange day / blue night) at trailing edge. Profile button (SF Symbol `person.fill`, 42×42 glass circle) + search bar (magnifyingglass + "Search" text, glass capsule) wrapped in `GlassEffectContainer(spacing: 12)`. Title "My Dreams" (`.fixedSize()`) + `EmotionFilterBar` with `Spacer(minLength: 24)` between them. VStack spacing 20pt between profile/search row and title/filters row.
- **Emotion filter bar:** `EmotionFilterBar` with collapsed/expanded states + dynamic reordering. `@State emotionOrder` tracks MRU order — selected emotion moves to front, others keep relative positions. Fixed frame width = collapsed layout width, `.mask` clips left edge only (right extends 40pt for no clip). Collapsed: `HStack(spacing: -14)`, `zIndex` by position (first on top). Expanded: `ScrollView(.horizontal)` + `HStack(spacing: 4)`, NO `GlassEffectContainer` (it draws glass over dimming overlays). Each emotion: `Image(emotion.iconName)` 30×30 in 42×42 frame. ZStack layers: dark base `Circle(Color(white: 0.15))` → image + `.reveriGlass(.circle, interactive: false)` → dimming `Circle(.black.opacity(0.55))` with `.id("dim-\(emotion)-\(selected)")` for instant (non-animated) appearance. `.clipShape(Circle())` on ZStack. `DreamEmotion.iconName` maps to PNG assets, `DreamEmotion.journalIcon` maps to colored tab SVGs, `DreamEmotion.color` provides HEX colors.
