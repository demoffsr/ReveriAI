  ...
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      if (!file || !(file instanceof File)) {
        return new Response(JSON.stringify({ error: 'Missing audio file', requestId }), { status: 400, headers: corsHeaders })
      }
    ...
    if (audioData.length === 0) {
      return new Response(JSON.stringify({ error: 'Empty audio data', requestId }), { status: 400, headers: corsHeaders })
    }
    ...
    if (!openaiKey) {
      console.error(`[transcribe-audio][${requestId}] Missing required configuration`)
    ...
    if (!response.ok) {
      ...
      console.error(`[transcribe-audio][${requestId}] AI service error (${response.status}): ${errorText}`)
    ...
    console.error(`[transcribe-audio][${requestId}] Unhandled error:`, err)
