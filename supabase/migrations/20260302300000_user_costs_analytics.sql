...
-- Aggregates cost_usd from ai_*_completed event metadata

-- Drop old functions that return json (we need jsonb)
DROP FUNCTION IF EXISTS analytics_user_list(int, int, int);
DROP FUNCTION IF EXISTS analytics_user_profile(text);

-- Detailed cost breakdown per user
...
DECLARE
  ...
  v_user_id uuid;
BEGIN
  -- Overall costs (all users or single user)
...
-- Update analytics_user_profile to include cost breakdown
